...

{
  ...
  nodes = {
    ...
    router = { config, pkgs, ... }:
    {
      ...
      systemd.services."jool" = {
        description = "jool with netfilter";
        ...
        serviceConfig = {
          ...
          ExecStart = "${pkgs.jool-cli}/bin/jool instance add default --netfilter --pool6 64:ff9b::/96";
          ExecStop = "${pkgs.jool-cli}/bin/jool --instance default remove";
        };
      };
      ...
      #  };
      #};
      environment.systemPackages = [ pkgs.mtr pkgs.jool-cli pkgs.tcpdump ];
  ...
  testScript = ''
    ...
    for machine in client, server, router:
    ...
    #router.wait_for_unit("jool.service")
    ...
    router.log(router.execute("systemd-analyze security jool.service")[1])
  '';
})
