...
in
{
  ...
  config = mkIf cfg.enable {
    systemd.network = {
      ...
      networks."${cfg.tunDevice}" = {
        name = cfg.tunDevice;
        addresses = [
          { addressConfig = { Address = cfg.ipv4.router.address+"/32"; }; }
          { addressConfig = { Address = cfg.ipv6.router.address+"/128"; }; }
        ];
        routes = [
          { routeConfig = { Destination = strAddr cfg.ipv4.pool; }; }
          { routeConfig = { Destination = strAddr cfg.ipv6.pool; }; }
        ];
